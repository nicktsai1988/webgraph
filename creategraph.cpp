...
void CreateGraph::create()
{
    ...
    unsigned int i,j;
    for(i=1;i<=exist;i++)//先初始化存在的页面，然后再根据存在的页面生成其它页面
    {
...
{
    unsigned int temp=(rand()%(graph.nodecount-1))+1; //分配ID为1~(nodecount-1)给链接
    while(have_existed(links,size,temp))  //若不存在某个链接则返回，否则继续选择
    {
...
unsigned int CreateGraph::new_link(const unsigned int *links, const unsigned int size, unsigned int seed)
{
    ...
    {
        unsigned int temp2=(rand()%(graph.nodecount-1))+1; //取出去除节点ID为０的节点
