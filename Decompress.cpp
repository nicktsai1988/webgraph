...
int DeCompress::getlinks(unsigned int pageid,std::vector<unsigned int>& result)
{
	...
	cerr<<endl;
	*/
	CacheMap::iterator cacheindex;
	vector<DeCode> decodelist;
	DeCode onenode;
	unsigned int position,length=0;
	do
	{
		cacheindex=cache.find(pageid);//查看节点是否存在于cache中
		if(cacheindex != cache.end())//存在
			{
				onenode.nodeid=pageid;
				onenode.outdegree=(cacheindex->second).size();
				onenode.reference=0;
				onenode.differences.clear();
				onenode.links=cacheindex->second;
...
void DeCompress::insertcache(const DeCode& newnode)
{
	...
	if((it=cache.find(newnode.nodeid)) != cache.end())
		return;//该节点已经存在于cache中
	if(cache.size()<cachesize)//cache内元素数量小于限制
	{
		cache.insert(CacheMap::value_type(newnode.nodeid,newnode.links));
		return;
	}
	...
	//node.nodeid=newnode.nodeid;
//	node.links=newnode.links;
	CacheMap::iterator index;
	double value=0,temp;
	for(it=cache.begin();it != cache.end();++it)
	{
		temp=similarity(newnode.links,it->second);
		if(temp<limit)
		{
			if(temp>value)
			{
	...
	if(it == cache.end())//不存在相似度>=limit的点存在,则进行替换
	{
		cache.erase(index);
		cache.insert(CacheMap::value_type(newnode.nodeid,newnode.links));
	}
...
/* 
CacheMap::iterator DeCompress::page_existed(unsigned int pageid)//此处可以使用二分查找，或者哈系表
{
		return cache.find(pageid);
}
*/
